/*
 * Ported verbatim from HyperFrames (Apache License 2.0).
 * Source: packages/studio/src/player/store/playerStore.ts
 *
 * The C.A.D.S. player store. Holds the TimelineElement shape, the ZoomMode
 * type, and the `liveTime` pub-sub used to drive the RAF-loop playhead without
 * triggering a React re-render on every frame (mandatory per spec constraints).
 * Logic is unchanged; only the import path is adapted.
 *
 * Copyright HeyGen / HyperFrames contributors.
