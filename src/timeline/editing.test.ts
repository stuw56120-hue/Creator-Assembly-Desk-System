/*
 * Ported from HyperFrames (Apache License 2.0).
 * Source: packages/studio/src/player/components/timelineEditing.test.ts
 * Additions for C.A.D.S.: explicit min-duration (0.1s) resize coverage.
 *
...
describe("resolveTimelineResize", () => {
  ...
  // ── C.A.D.S. additions: min duration 0.1s floor ──────────────────────────
  it("does not let an end-resize shrink a clip below the 0.1s default minimum", () => {
    // Drag the right edge far left (well past the clip start); duration should
    // clamp to the default minimum of 0.1s rather than going to zero/negative.
    const result = resolveTimelineResize(
      {
        start: 1,
        duration: 3,
        originClientX: 1000,
  ...
  it("does not let a start-resize shrink a clip below the 0.1s default minimum", () => {
    // Drag the left edge far right (past the clip end); the start advances but
    // duration floors at 0.1s, so start = (start + duration) - 0.1 = 3.9.
    const result = resolveTimelineResize(
      {
        start: 1,
        duration: 3,
    ...
    expect(result.duration).toBe(0.1);
    expect(result.start).toBe(3.9);
  });

  it("honours an explicit minDuration override above the default", () => {
    const result = resolveTimelineResize(
      {
        start: 0,
        duration: 5,
